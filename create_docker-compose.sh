...
station_name[$station_number]=$(echo "${body_station}" | jq -r .stations[$station_number].name | sed 's/ /\_/g' | sed 's/.*/\L&/' | sed 's|[<>,]||g')
station_id[$station_number]=$(echo "${body_station}" | jq .stations[$station_number].station_id)
device_id[$station_number]=$(echo "${body_station}" | jq .stations[$station_number].devices[1].device_id)
...
services:
  ...
  weatherflow-collector-${station_name[$station_number]}-local-udp:
    ...
    restart: always
    depends_on:
      - \"wxfdashboardsaio_influxdb\"" > docker-compose.yml
...
echo "
  weatherflow-collector-${station_name[$station_number]}-remote-forecast:
    container_name: weatherflow-collector-${station_name[$station_number]}-remote-forecast
    environment:
      WEATHERFLOW_COLLECTOR_BACKEND_TYPE: influxdb
  ...
  weatherflow-collector-${station_name[$station_number]}-remote-rest:
    container_name: weatherflow-collector-${station_name[$station_number]}-remote-rest
    environment:
      WEATHERFLOW_COLLECTOR_BACKEND_TYPE: influxdb
  ...
  weatherflow-collector-${station_name[$station_number]}-remote-socket:
    container_name: weatherflow-collector-${station_name[$station_number]}-remote-socket
    environment:
      WEATHERFLOW_COLLECTOR_BACKEND_TYPE: influxdb
